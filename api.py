        ...
        if os.path.isdir(image_path):
            ...
            for image_path in os.listdir(image_folder):
                ...
                with torch.no_grad():
                    ...
                    pred_mask = (pred.cpu().numpy()[0,0,:,:]>0.5).astype(np.uint8)*255
                    print('Export mask')
                    cv2.imwrite('./debug/original_mask.jpg',pred_mask)
                    dilation = cv2.dilate(pred_mask,np.ones((5,5),np.uint8),iterations = 1)
