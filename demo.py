        ...
        if os.path.isdir(image_path):
            ...
            for image_path in os.listdir(image_folder):
                ...
                with torch.no_grad():
                    ...
                    pred_border=(pred_border.cpu().numpy()[0,0,:,:]>0.001).astype(np.uint8)*255
                    output = self.structure.representer.represent(batch, pred, is_output_polygon=self.args['polygon'],img=image_path) 
                    ...
                    if True and self.structure.visualizer:
                        ...
                        cv2.imwrite(os.path.join(self.args['result_dir'], image_path.split('/')[-1].split('.')[0]+'_pred_border.jpg'), pred_border)
                        print(np.max(predict_map))
                        print(vis_image.shape)
                        predict_map=cv2.resize(predict_map,(vis_image.shape[1],vis_image.shape[0]))
                        predict_map_debug=np.stack([255-predict_map,
                                                    255-predict_map,
                                                    predict_map],axis=2)+vis_image
                                                    
                                                    
                        predict_map_debug=np.clip(predict_map_debug, 0, 255)
                        print("====== saving file to===",os.path.join(self.args['result_dir'],image_path.split('/')[-1].split('.')[0]+'_debug.jpg'))
                        cv2.imwrite(os.path.join(self.args['result_dir'], image_path.split('/')[-1].split('.')[0]+'_debug.jpg'), predict_map_debug)
