...
from .DMFNet_16x import MFunit, DMFUnit
...
class BiFPNUnit(nn.Module):
    def __init__(self, n=32, channels=128, groups=16, norm='bn', base_unit=MFunit, **kwargs):
        ...
        self.conv12 = base_unit(channels, n, g=groups, stride=1, norm=norm)
        ...
        # self.conv21 = nn.Conv2d(3 * channels, channels, g=groups, stride=1, norm=norm)
        # self.conv31 = nn.Conv2d(4 * channels, 2 * channels, g=groups, stride=1, norm=norm)
        # self.conv12 = nn.Conv2d(n + channels, n, g=groups, stride=1, norm=norm)
        # self.conv22 = nn.Conv2d(n + 2 * channels, channels, g=groups, stride=1, norm=norm)
        # self.conv32 = nn.Conv2d(5 * channels, 2 * channels, g=groups, stride=1, norm=norm)
        # self.conv42 = nn.Conv2d(4 * channels, 2 * channels, g=groups, stride=1, norm=norm)
        
        self.upsample21 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False)
        self.upsample32 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False)
        self.upsample43 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False)

        self.downsample12 = nn.MaxPool2d(kernel_size=2)
        self.downsample23 = nn.MaxPool2d(kernel_size=2)
        self.downsample34 = nn.MaxPool2d(kernel_size=2)
