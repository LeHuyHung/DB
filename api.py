...
class Demo:
    ...
    def inference(self, image_path, visualize=False,reid=None,box=None):
        ...
        if os.path.isdir(image_path):
            ...
            for image_path in os.listdir(image_folder):
                ...
                with torch.no_grad():
                    ...
                    if True and self.structure.visualizer:
                        ...
                        cv2.imwrite(os.path.join(self.args['result_dir'], image_path.split('/')[-1].split('.')[0]+str(time.time())+'_score.jpg'), predict_map)
            ...
            with torch.no_grad():
                ...
                border_dilation = cv2.dilate((pred_border.cpu().numpy()[0,0,:,:]>0.5).astype(np.uint8)*255,np.ones((5,5),np.uint8),iterations = 1)
                border_dilation=skeletonlize(border_dilation)
                border_dilation=cv2.dilate(border_dilation,np.ones((3,3),np.uint8),iterations = 1)
                ...
                if visualize and self.structure.visualizer:
                    vis_image = self.structure.visualizer.demo_visualize(image_path, output)
                    cv2.imwrite(os.path.join(self.args['result_dir'], image_path.split('/')[-1].split('.')[0]+'_visualize.jpg'), vis_image)
                    cv2.imwrite(os.path.join(self.args['result_dir'], image_path.split('/')[-1].split('.')[0]+'_border.jpg'), dilation_border)
                    dilation_border=cv2.resize(dilation_border,(vis_image.shape[1],vis_image.shape[0]))
                    vis_image[:,:,0]-=dilation_border
                    vis_image[:,:,1]-=dilation_border
                    vis_image[:,:,2]+=dilation_border
                    vis_image= np.clip(vis_image, 0, 255)
                    cv2.imwrite(os.path.join(self.args['result_dir'], image_path.split('/')[-1].split('.')[0]+'_full.jpg'), vis_image)
...
def contour_intersect(cnt_ref,cnt_query, edges_only = False):
    ...
    for pt in cnt_query:
        ...
        if edges_only and (cv2.pointPolygonTest(cnt_ref,(x,y),True) == 0):
            intersecting_pts.append(pt)
        elif not(edges_only) and (cv2.pointPolygonTest(cnt_ref,(x,y),True) >= 30):
            intersecting_pts.append(pt)
...
def content_post():
    try:
        ...
        reid = request.form.get('reid', "temp_reid")
        box = [request.form.get('box_x1', None),request.form.get('box_y1', None),request.form.get('box_x2', None),request.form.get('box_y2', None)]
        print(reid,box)
        bio = io.BytesIO()
        ffile.save(bio)
        ...
        if os.path.exists("temp") :
            shutil.rmtree("temp")
        os.mkdir("temp")
        cv2.imwrite("temp/"+reid+"_"+str(box)+".jpg",image)
        formated_output=inference_endpoint.inference("temp/"+reid+"_"+str(box)+".jpg", args['visualize'],reid,box)
        
        if box[0] is not None:
            clear_formated_output=[]
            print('box',box)
            print(type(box))
            for countour in formated_output:
                print(countour)
                if contour_intersect(np.array([[box[0],box[1]],
                                            [box[0],box[3]],
                                            [box[2],box[3]],
                                            [box[2],box[1]]],dtype=np.int32),
                                            np.array(countour[0],dtype=np.int32)):
                    countour[0]=(np.array(countour[0],dtype=np.int32)-np.array([[box[0],box[1]]],dtype=np.int32)).tolist()
                    clear_formated_output.append(   countour)                             
            formated_output=clear_formated_output    
        status_code=200
        response = {
            'status': 'OK',
