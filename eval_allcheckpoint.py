...
def main():
    ...
    for checkpoint_path in os.listdir(checkpoint_dir):
        args['resume']=os.path.join(checkpoint_dir,checkpoint_path)
    ...
    def eval(self, visualize=False):
        ...
        with torch.no_grad():
            for _, data_loader in self.data_loaders.items():
                ...
                with open("evaluate_score.csv","a") as f:
                    f.write(self.checkpoint_path)
                    key="precision"
                    metric=metrics[key]
                    f.write(","+str(metric.avg))
