...
def getSubImage(rect, src):
    ...
    cv2.imwrite("debug/full_img/"+str(len(out))+"_des.jpg",dst)
    print(rect)
    return out
...
debug={"full_img":True, "crop_img":True,"binary_img":True,"recognition":False,"shrink":True,"split_long_box":True}
...
def content_post():
    try:
        ...
        for polygon_data in detection_result['data']:
            ...
            # recognition_images.append({"box_rec":box_rec,"image":crop_minAreaRect(image, rotrect)})
            recognition_images.append({"box_rec":box_rec,"image":getSubImage(rotrect, image)})
        ...
        # ============ crop image output debug ==============
            ...
            for polygon_data in detection_result['data']:
                ...
                rotrect = cv2.minAreaRect(np.array(poly,dtype=np.int32))
                box_rec = np.array(cv2.boxPoints(rotrect), np.int32)

                cv.drawContours(image_,[np.array(poly)],-1,20, thickness=1)
                # cv.drawContours(image_,[box_rec],-1,180,2)
            cv2.imwrite("debug/full_img/"+str(len(poly))+".jpg",image_)
        ...
        # ============ visualize OCR mix ==============
            ...
            for crop_id,croped_img_data in enumerate(recognition_images):
                ...
                draw.text((min(box_rec[:,0]),min(box_rec[:,1])), text, font=font,fill=(255,255,255,128))
